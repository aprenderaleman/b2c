...
async function run(req: Request) {
  ...
  for (const r of (classes ?? []) as Row[]) {
    ...
    if (lead.email) {
      const res = await sendTrialReminderEmail(lead.email, {
        ...
        joinUrl:         leadJoinUrl,
        confirmUrl:      buildEmailActionUrl({ leadId: lead.id, classId: r.id, action: "confirm" }),
  ...
  // Citas del mismo día quedan fuera de la ventana 23-25h → skip natural.
  const sesiones = await sendSesionReminders(sb, "24h").catch(() => 0);
