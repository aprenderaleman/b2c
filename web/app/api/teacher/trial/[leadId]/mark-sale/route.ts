...
export async function POST(
  ...
  const { data: venta, error } = await sb
    ...
    .insert({
      lead_id: leadId,
      solicitado_por: teacherId,
      rol_solicitante: "teacher",
  ...
  await sb.from("lead_timeline").insert({
    ...
    type: "agent_note",
    author: "teacher",
    content: `Profesor marco venta (pack: ${parsed.data.packId}, ${parsed.data.paymentType}). Pendiente de aprobacion.`,
    metadata: {
      kind: "sale_marked",
      venta_id: venta.id,
  ...
  await createAdminNotification({
    ...
    body: `${teacherName} marco venta para ${lead.full_name ?? "lead"} (pack ${parsed.data.packId})`,
    lead_id: leadId,
