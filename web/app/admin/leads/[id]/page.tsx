    ...
    <main className="space-y-5">
      ...
      <header className="rounded-3xl border border-slate-200 bg-white p-5">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="min-w-0">
            <h1 className="text-2xl font-bold text-slate-900">{lead.name || "Unnamed lead"}</h1>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <div className="space-y-5 lg:col-span-1">
          <Panel title="Funnel data">
            <Kv k="Created"          v={new Date(lead.created_at).toLocaleString("de-DE")} />
            <Kv k="Source"           v={lead.source} />
            <Kv k="German level"     v={lead.german_level} />
            <Kv k="Goal"             v={lead.goal} />
            <Kv k="Urgency"          v={lead.urgency} />
            <Kv k="Budget"           v={lead.budget ?? "—"} />
            <Kv k="Email"            v={lead.email ?? "—"} />
            <Kv k="Messages seen"    v={String(lead.messages_seen_count)} />
            <Kv k="Follow-up #"      v={String(lead.current_followup_number)} />
            <Kv k="Next contact"     v={lead.next_contact_date ? new Date(lead.next_contact_date).toLocaleString("de-DE") : "—"} />
            <Kv k="Trial scheduled"  v={lead.trial_scheduled_at ? new Date(lead.trial_scheduled_at).toLocaleString("de-DE") : "—"} />
            {lead.trial_zoom_link && <Kv k="Trial link" v={lead.trial_zoom_link} />}
            <Kv k="GDPR accepted"    v={lead.gdpr_accepted ? `Yes · ${lead.gdpr_accepted_at ? new Date(lead.gdpr_accepted_at).toLocaleDateString("de-DE") : ""}` : "No"} />
          </Panel>

          <Panel title="Gelfis notes">
            <form
            ...
            >
              <textarea
                ...
                maxLength={2000}
                rows={3}
                placeholder="Add a note (never deleted)…"
                className="input-text"
              />
              <button type="submit" className="btn-primary text-sm">Add note</button>
            </form>
            <ul className="mt-4 divide-y divide-slate-100">
              {notes.length === 0 && <li className="py-2 text-sm text-slate-500">No notes yet.</li>}
              {notes.map((n) => (
                <li key={n.id} className="py-2">
                  <div className="text-xs text-slate-500">{new Date(n.created_at).toLocaleString("de-DE")}</div>
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <a
        ...
        title="GDPR: download all data about this lead"
      >
        Export (JSON)
      </a>
  ...
  return (
    <li className="py-3">
      <div className="flex items-center gap-2 text-xs">
        <span className={`rounded-full border px-2 py-0.5 font-medium ${cls}`}>{entry.type}</span>
        <span className="text-slate-500">{entry.author}</span>
        <span className="ml-auto text-slate-400">{new Date(entry.timestamp).toLocaleString("de-DE")}</span>
      </div>
