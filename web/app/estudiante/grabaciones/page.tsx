import Link from "next/link";
import { requireRoleWithImpersonation } from "@/lib/rbac";
import { getStudentByUserId } from "@/lib/academy";
import { getStudentRecordings, formatDurationHms } from "@/lib/recordings";
import { formatClassDateEs, formatClassTimeEs } from "@/lib/classes";

export const dynamic = "force-dynamic";
export const metadata = { title: "Grabaciones · Aprender-Aleman.de" };

/**
 * Lista todas las grabaciones de las clases (individuales y de grupo) en
 * las que participó el estudiante. La consulta ya está implementada
 * en lib/recordings.getStudentRecordings — esta página sólo la pinta.
 *
 * Agrupa visualmente por mes para que sea cómodo navegar el histórico
 * cuando el grupo lleva varios meses dando clase.
 */
export default async function StudentRecordingsPage() {
  // Same pattern as /estudiante/clases — when an admin uses "Ver como
  // estudiante" the session.user.id resolves to the impersonated student.
  const session = await requireRoleWithImpersonation(
    ["student", "admin", "superadmin"],
    "student",
  );
  const student = await getStudentByUserId(session.user.id);
  if (!student) {
    return (
      <main className="mx-auto max-w-3xl p-6">
        <h1 className="text-2xl font-semibold">Grabaciones</h1>
        <p className="mt-3 text-slate-600 dark:text-slate-300">
          Tu cuenta no está asociada a un estudiante. Contacta con la academia.
        </p>
      </main>
    );
  }

  const recs = await getStudentRecordings(student.id);

  // Agrupa por mes (YYYY-MM en zona Berlín) — usa scheduled_at, no created_at
  // de la grabación, para que el orden case con la cronología real de las
  // clases.
  const byMonth = new Map<string, typeof recs>();
  for (const r of recs) {
    const key = monthKeyBerlin(r.scheduled_at);
    const arr = byMonth.get(key) ?? [];
    arr.push(r);
    byMonth.set(key, arr);
  }
  const monthsDesc = [...byMonth.keys()].sort((a, b) => b.localeCompare(a));

  return (
    <main className="mx-auto max-w-4xl p-4 sm:p-6 space-y-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-50">
          Mis grabaciones
        </h1>
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
          Todas las clases tuyas que se grabaron y ya están listas. Las grabaciones
          se procesan después de cada clase y suelen estar disponibles en menos de 1h.
        </p>
      </header>

      {recs.length === 0 && (
        <div className="rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 p-8 text-center">
          <div className="text-4xl">🎬</div>
          <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">
            Aún no hay grabaciones disponibles. Aparecerán aquí en cuanto se procesen.
          </p>
        </div>
      )}

      {monthsDesc.map(mk => (
        <section key={mk} className="space-y-2">
          <h2 className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
            {labelForMonth(mk)}
          </h2>
          <ul className="space-y-2">
            {(byMonth.get(mk) ?? []).map(r => (
              <li key={r.id} className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3 sm:p-4 shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                  <div className="min-w-0">
                    <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100 truncate">
                      {r.class_title}
                    </h3>
                    <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400 capitalize">
                      {formatClassDateEs(r.scheduled_at)} · {formatClassTimeEs(r.scheduled_at)}
                      {r.duration_seconds ? ` · ${formatDurationHms(r.duration_seconds)}` : ""}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <StatusBadge status={r.status} />
                    {r.status === "ready" && (
                      <Link
                        href={`/grabacion/${r.id}`}
                        prefetch={false}
                        className="btn-primary text-xs whitespace-nowrap"
                      >
                        ▶ Reproducir
                      </Link>
                    )}
                  </div>
                </div>
              </li>
            ))}
          </ul>
        </section>
      ))}
    </main>
  );
}

function StatusBadge({ status }: { status: "processing" | "ready" | "failed" }) {
  if (status === "ready") return null;   // si está lista, basta el botón
  const styles =
    status === "processing" ? "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300"
                            : "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300";
  const text = status === "processing" ? "Procesando…" : "Falló";
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold ${styles}`}>
      {text}
    </span>
  );
}

function monthKeyBerlin(iso: string): string {
  const d = new Date(iso);
  const fmt = new Intl.DateTimeFormat("en-CA", {
    year: "numeric", month: "2-digit", timeZone: "Europe/Berlin",
  });
  return fmt.format(d);   // "2026-05"
}

function labelForMonth(key: string): string {
  const [Y, M] = key.split("-").map(Number);
  const d = new Date(Date.UTC(Y, M - 1, 15));
  return d.toLocaleDateString("es-ES", { month: "long", year: "numeric", timeZone: "Europe/Berlin" })
          .replace(/^\w/, c => c.toUpperCase());
}
