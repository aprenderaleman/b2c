...
import { SesionHubCard, type SesionRow } from "./SesionHubCard";

type Tab = "upcoming" | "history";

type SesionRowWithCloser = SesionRow & { closerName?: string };

export function SesionHubShell({
  upcoming,
  past,
  isAdmin = false,
}: {
  upcoming: SesionRowWithCloser[];
  past: SesionRowWithCloser[];
  isAdmin?: boolean;
}) {
  ...
  const filtered = useMemo(() => {
    ...
    return source.filter((r) => {
      const haystack = [
        ...
        r.leadGoal,
        r.leadGermanLevel,
        (r as SesionRowWithCloser).closerName,
      ]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
          ...
          <div className="space-y-3">
            {filtered.map((s) => (
              <SesionHubCard
                ...
                fichaHref={isAdmin ? `/admin/leads/${s.leadId}` : undefined}
              />
            ))}
          </div>
        </>
