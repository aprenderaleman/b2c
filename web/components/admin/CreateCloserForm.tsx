...
export function CreateCloserForm() {
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      ...
      if (!res.ok) {
        ...
        setError(body?.message ?? body?.error ?? "Error al crear el closer.");
        return;
      }
      const body = await res.json().catch(() => ({}));
    ...
    >
      ...
      {tempPassword && (
        <div className="rounded-2xl border border-amber-300 dark:border-amber-500/40 bg-amber-50 dark:bg-amber-500/10 p-4 text-sm text-amber-800 dark:text-amber-200">
          <strong>Closer creado</strong>, pero el email de bienvenida NO se pudo enviar. Pasale estas credenciales manualmente:
          <br />
      ...
      <button type="submit" className="btn-primary" disabled={pending}>
        {pending ? "Creando..." : "Crear closer y enviar acceso"}
      </button>
    </form>
  );
}
